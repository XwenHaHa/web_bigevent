  ...
  $('body').on('submit', '#formadd', function (e) {
    ...
    $.ajax({
      ...
      success: function (res) {
        ...
        if (
          res.status === 1 &&
          res.message === '分类名称与别名被占用，请稍候重试'
        ) {
          layer.msg('分类名称与别名被占用，请稍候重试!')
          // 根据索引关闭弹出层
          layer.close(indexAdd)
        } else if (
          res.status === 1 &&
          res.message === '分类名称被占用，请稍候重试'
        ) {
          layer.msg('分类名称被占用，请稍候重试!')
          // 根据索引关闭弹出层
          layer.close(indexAdd)
        } else if (
          res.status === 1 &&
          res.message === '分类别名被占用，请稍候重试'
        ) {
          layer.msg('分类别名被占用，请稍候重试!')
          // 根据索引关闭弹出层
          layer.close(indexAdd)
        } else {
          layer.msg('新增文章分类成功！')
